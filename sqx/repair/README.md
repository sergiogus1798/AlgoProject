# sqx/repair — write to a project on disk, once SQX is not holding it
...
|---|---|---|---|
| `graft_tasks.py` | Heal a project whose archive is missing task files its `config.xml` declares, taking only the absent members from a donor archive | `python3 -m sqx.repair.graft_tasks <PROJECT> [--apply]` | broken `.cfx` → repaired `.cfx` |
